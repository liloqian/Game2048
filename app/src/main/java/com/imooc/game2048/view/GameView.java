...
public class GameView extends GridLayout implements OnTouchListener {
    ...
    public void startGame() {
        initGameMatrix();
        //initGameView(Config.mItemSize);  //这里可以注释
    }

    /**数字全部初始化为0 ， 然后随机初始化两个数字*/
    ...
    private void addRandomNum() {
        ...
        if (mBlanks.size() > 0) {
            ...
            mGameMatrix[randomPoint.x][randomPoint.y]
                    .setNum(Math.random() > 0.2d ? 2 : 4);  //80%是2  20%是4
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                checkCompleted();  //判断游戏是否达到目标或者已经失败
                break;
            default:
                break;
        }
    ...
    private void checkCompleted() {
        ...
        if (result == 0) {
            if (Config.SCROE > mHighScore) {        //保存新纪录
    ...
    private void swipeRight() {
        for (int i = mGameLines - 1; i >= 0; i--) {
            ...
            for (int j = mGameLines - 1; j >= 0; j--) {
                ...
                if (currentNum != 0) {
                    if (mKeyItemNum == -1) {
                        ...
                        if (mKeyItemNum == currentNum) {  //两个一样的
                            mCalList.add(mKeyItemNum * 2);
                            Config.SCROE += mKeyItemNum * 2;
                            mKeyItemNum = -1;
                        } else {
            ...
            // 改变Item值
            for (int j = 0; j < mGameLines - mCalList.size(); j++) {  //前面的设为0
